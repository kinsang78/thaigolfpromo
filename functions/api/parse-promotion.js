...
// onRequestPost = POST 요청만 받음 (카카오 챗봇 Webhook)
// ====================================================================
export async function onRequestPost(context) {
  const { request, env } = context;

  // 환경변수에서 클라이언트 생성 (요청마다 생성: Workers는 글로벌 state 제한)
  ...
  try {
    ...
    const masterListText = Object.entries(COURSE_MASTER)
      .filter(entry => entry[1].en && entry[0] !== entry[1].en)
      .map(entry => entry[0] + " -> " + entry[1].en + " | " + entry[1].country + " | " + entry[1].region)
      .join("\n");
    ...
    promotions = promotions.map(promo => correctWithMaster(promo, COURSE_MASTER));
    console.log("파싱 결과:", promotions.length, "건");

    const results = { saved: [], updated: [], skipped: [] };
    for (let i = 0; i < promotions.length; i++) {
      const saveResult = await saveWithDedup(supabase, promotions[i], utterance, userId);
      results[saveResult.action].push(promotions[i]);
    }

    // 만료 정리는 백그라운드로 (Cloudflare Workers는 waitUntil 사용 권장)
    context.waitUntil(cleanupExpired(supabase).catch(e => console.error("만료 정리 실패:", e.message)));
    ...
    if (results.updated.length > 0) {
      ...
      for (let u = 0; u < results.updated.length; u++) {
    ...
    if (results.skipped.length > 0) {
      ...
      for (let s = 0; s < results.skipped.length; s++) {
...
async function loadCourseMaster(supabase) {
  ...
  result.data.forEach(c => {
    if (c.name_ko) {
      master[c.name_ko] = {
        id: c.id, en: c.name_en, country: c.country, region: c.region,
...
function correctWithMaster(promo, COURSE_MASTER) {
  ...
  if (COURSE_MASTER[name]) return applyMaster(promo, COURSE_MASTER[name]);

  const keys = Object.keys(COURSE_MASTER);
  let bestMatch = null, bestLen = 0;
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    if (key.length < 2) continue;
    if (name.indexOf(key) !== -1 || key.indexOf(name) !== -1) {
      const matchLen = Math.min(name.length, key.length);
      if (matchLen > bestLen) { bestLen = matchLen; bestMatch = key; }
    }
  }
  ...
  if (promo.golf_course_en) {
    ...
    for (let j = 0; j < keys.length; j++) {
      ...
      const masterEn = COURSE_MASTER[keys[j]].en.toLowerCase();
...
function cleanRegion(promo) {
  ...
  if (promo.region === "태국" || promo.region === "인도네시아") promo.region = null;
  else if (promo.region.indexOf("태국 ") === 0) promo.region = promo.region.replace("태국 ", "");
  else if (promo.region.indexOf("인도네시아 ") === 0) promo.region = promo.region.replace("인도네시아 ", "");
}
...
function formatSummary(p) {
  const curr = p.currency === "KRW" ? "원" : p.currency === "IDR" ? "루피아" : "바트";
  ...
  const prices = p.green_fee ? Object.entries(p.green_fee).map(e => {
    const label = { default:"", weekday:"주중 ", weekend:"주말 ", morning:"오전 ", afternoon:"오후 " }[e[0]] || "";
