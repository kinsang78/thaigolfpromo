...
function buildSystemPrompt(today, firstDay, lastDay, masterListText) {
  return [
    ...
    "2. **대회(단일 날짜)**: 텍스트에 '대회', '토너먼트', '샷건', '시상', '상금', '행사' 등의 키워드가 있다면, 이는 하루짜리 단일 행사입니다. 반드시 start_date와 end_date를 행사일 하루로 똑같이 설정하고, conditions 맨 앞에 '[대회]' 태그를 붙이세요.",
    "3. **이벤트(단일 날짜)**: 텍스트에 대회 관련 키워드가 없다면, 이는 하루짜리 특별 할인 행사입니다. 반드시 start_date와 end_date를 행사일 하루로 똑같이 설정하세요., conditions 맨 앞에 '[이벤트]' 태그를 붙이세요.",
    "",
